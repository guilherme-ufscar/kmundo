  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      <div className="mb-5 sm:mb-8">
        ...
        <p className="text-sm" style={{ color: '#6B7280' }}>Bem-vinda de volta ao seu painel</p>
      </div>
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-4 sm:mt-6 mb-5 sm:mb-8">
        <div className="bg-white rounded-2xl p-3 sm:p-5" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div className="flex items-start justify-between mb-2 sm:mb-3">
            <span className="text-xs sm:text-sm font-medium leading-tight" style={{ color: '#6B7280' }}>Comprado</span>
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center shrink-0 ml-1" style={{ background: '#FFF1F5' }}>
              <Package className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#FF6B9D' }} />
            </div>
          </div>
          <p className="text-2xl sm:text-3xl font-bold" style={{ color: '#1A1A2E' }}>{emArmazem}</p>
          <p className="text-xs mt-1 hidden sm:block" style={{ color: '#9CA3AF' }}>aguardando envio</p>
        </div>

        <div className="bg-white rounded-2xl p-3 sm:p-5" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div className="flex items-start justify-between mb-2 sm:mb-3">
            <span className="text-xs sm:text-sm font-medium leading-tight" style={{ color: '#6B7280' }}>Em Trânsito</span>
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center shrink-0 ml-1" style={{ background: '#FFFBEB' }}>
              <Truck className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#F59E0B' }} />
            </div>
          </div>
          <p className="text-2xl sm:text-3xl font-bold" style={{ color: '#1A1A2E' }}>{emEnvio}</p>
          <p className="text-xs mt-1 hidden sm:block" style={{ color: '#9CA3AF' }}>a caminho</p>
        </div>

        <div className="bg-white rounded-2xl p-3 sm:p-5" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div className="flex items-start justify-between mb-2 sm:mb-3">
            <span className="text-xs sm:text-sm font-medium leading-tight" style={{ color: '#6B7280' }}>Entregues</span>
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center shrink-0 ml-1" style={{ background: '#F0FDF4' }}>
              <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: '#22C55E' }} />
            </div>
          </div>
          <p className="text-2xl sm:text-3xl font-bold" style={{ color: '#1A1A2E' }}>{entregues}</p>
          <p className="text-xs mt-1 hidden sm:block" style={{ color: '#9CA3AF' }}>total recebidos</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100">
          ...
          <Link href="/meus-itens" className="text-sm font-medium hover:opacity-80" style={{ color: '#FF6B9D' }}>
            Ver todos →
          </Link>
        </div>
          ...
          <>
            ...
            <div className="sm:hidden divide-y divide-gray-50">
              {cliente.itens.map((item) => {
                ...
                return (
                  <Link key={item.id} href={`/meus-itens/${item.id}`} className="flex items-center gap-3 p-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm truncate" style={{ color: '#1A1A2E' }}>{item.descricao}</p>
                      <p className="text-xs mt-0.5" style={{ color: '#9CA3AF' }}>{item.lojaOrigem ?? '—'}</p>
                    </div>
